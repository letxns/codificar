...
codificar.addEventListener('click', function btnCodificar(){
    btnSubmit.innerText = "CODIFICAR";
    output.value = btoa(input.value);
    //let input = btoa(input.value)
   // let output = imput.value 
});

decodificar.addEventListener('click', function btnDecodificar(){
    btnSubmit.innerText = "DECODIFICAR";
...
/*
function base64(){
    let codificar = input.value;
    if(tipoSelecionado == "base64"){
        btn.addEventListener('click', (e) =>{
            input.value = btoa(output.value);
            e.preventDefault()
        });
    }
}*/
...
/*function cifraCesar(){
    var codificar = ((codigoASC - 65 + passos) % 26 + 65);
    return String.fromCharCode(codificar);
}*/

/*
function base64(){
    var criptografia = tipoCodigo.value
